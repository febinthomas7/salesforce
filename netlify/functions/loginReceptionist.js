...
export async function handler(event) {
  try {
    ...
    const { receptionist_id, password } = JSON.parse(event.body);
    ...
    const soql = `
      SELECT Id, Name, Email__c, Password_Hash__c , Hospital__c
      FROM Receptionist__c
      WHERE Receptionist_Id__c='${receptionist_id.replace(/'/g, "\\'")}'
      LIMIT 1
    `;
    ...
    const payload = {
      id: record.Id,
      name: record.Name,
      hospital_id: record.Hospital__c,
      role: "receptionist",
    };
