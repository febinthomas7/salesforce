...
const mockSalesforceService = {
  fetchData: async (hospitalId) => {
    ...
    const doctors = [
      { Id: 'DOC-001', Name: 'Dr. Sarah Smith', Department__c: 'Cardiology', Hospital_Ref__c: 'HOSP-001', Status__c: 'Available' },
      { Id: 'DOC-002', Name: 'Dr. James Wilson', Department__c: 'Neurology', Hospital_Ref__c: 'HOSP-001', Status__c: 'In Surgery' },
      { Id: 'DOC-003', Name: 'Dr. Anita Ray', Department__c: 'General Medicine', Hospital_Ref__c: 'HOSP-001', Status__c: 'Available' },
      { Id: 'DOC-004', Name: 'Dr. Robert Chen', Department__c: 'Dermatology', Hospital_Ref__c: 'HOSP-001', Status__c: 'On Leave' },
      { Id: 'DOC-005', Name: 'Dr. Emily Stone', Department__c: 'Cardiology', Hospital_Ref__c: 'HOSP-001', Status__c: 'Available' },
    ];
    ...
    const appointments = Array.from({ length: 25 }).map((_, i) => {
      ...
      return {
        Id: `APT-${1000 + i}`,
        Patient_Name__c: ['John Doe', 'Alice Johnson', 'Michael Brown', 'Emily Davis', 'Chris Wilson', 'Sarah Parker', 'Tom Hanks'][i % 7],
        Age__c: 20 + i * 2,
        Gender__c: i % 2 === 0 ? 'Male' : 'Female',
        Phone__c: `+91 98765 432${i % 10}`,
        Aadhaar__c: aadhaarRaw, 
        Department__c: ['Cardiology', 'General Medicine', 'Dermatology', 'Neurology'][i % 4],
        ...
        Date__c: dateString,
        Notes__c: dateOffset > 0 ? "Missed original slot, requesting reschedule." : "Walk-in patient registration via App."
      };
    });

    const hospitalDoctors = doctors.filter(d => d.Hospital_Ref__c === hospitalId);
    return { appointments, doctors: hospitalDoctors, hospitalName: CURRENT_HOSPITAL_NAME };
  }
};
...
const SidebarItem = ({ icon: Icon, label, active, onClick }) => (
  ...
  >
    <Icon size={20} className={active ? 'text-white' : 'group-hover:text-teal-600 transition-colors'} />
    <span className="font-medium">{label}</span>
    {active && <ChevronRight size={16} className="ml-auto opacity-75" />}
  </button>
);
...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span className={`px-3 py-1 rounded-full text-xs font-bold border ${styles[status] || styles.Pending} shadow-sm`}>
      {status}
    </span>
  );
};
...
const ReceptionistDashboard = () => {
  ...
  useEffect(() => {
    const initData = async () => {
      setLoading(true);
      try {
        const { appointments, doctors, hospitalName } = await mockSalesforceService.fetchData(CURRENT_HOSPITAL_ID);
  ...
  const filteredAppointments = useMemo(() => {
    ...
    return appointments.filter(apt => {
      ...
      // Search & Status
      const matchesSearch = apt.Patient_Name__c.toLowerCase().includes(searchTerm.toLowerCase()) || 
                            apt.Id.toLowerCase().includes(searchTerm.toLowerCase()) ||
                            apt.Aadhaar__c.replace(/\s/g, '').includes(searchTerm);
      const matchesStatus = selectedStatus === "All" || apt.Status__c === selectedStatus;
  ...
  const stats = {
    ...
    pending: filteredAppointments.filter(a => a.Status__c === 'Pending').length,
    successful: filteredAppointments.filter(a => a.Status__c === 'Successful').length,
  ...
  const getDoctorsForDepartment = (dept) => doctors.filter(doc => doc.Department__c === dept);
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex font-sans text-gray-800">
      ...
      {/* Sidebar - Desktop */}
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-gray-200 transform transition-transform duration-300 lg:translate-x-0 lg:static ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-6 flex items-center gap-3">
          <div className="w-10 h-10 bg-gradient-to-br from-teal-500 to-cyan-600 rounded-xl flex items-center justify-center text-white font-bold shadow-lg shadow-teal-200">
            M
          </div>
          <span className="text-xl font-extrabold tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-teal-700 to-cyan-600">
            MedLock
          </span>
        </div>

        <div className="px-6 mb-2">
            <div className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Main Menu</div>
        </div>

        <nav className="flex-1 px-4 space-y-2">
          <SidebarItem icon={Activity} label="Dashboard" active={true} onClick={() => {}} />
          <SidebarItem icon={Calendar} label="Appointments" active={false} onClick={() => {}} />
          <SidebarItem icon={Stethoscope} label="Doctors" active={false} onClick={() => {}} />
          <SidebarItem icon={Users} label="Patients" active={false} onClick={() => {}} />
        </nav>
        ...
        <div className="p-4 border-t border-gray-100 bg-gray-50/50">
          <div className="flex items-center gap-3 p-3 bg-white border border-gray-100 rounded-xl shadow-sm">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-gray-900 truncate">Reception Desk</p>
              <p className="text-xs text-gray-500 truncate">{CURRENT_HOSPITAL_ID}</p>
            </div>
      ...
      <main className="flex-1 min-w-0 overflow-hidden flex flex-col h-screen">
        ...
        <header className="bg-white border-b border-gray-200 px-8 py-5 flex justify-between items-center shrink-0">
            <div>
                <div className="flex items-center gap-2 mb-1">
                    <BuildingIcon className="text-teal-600" size={20} />
                    <h1 className="text-xl font-bold text-gray-800 tracking-tight">
                        {loading ? "Loading Hospital..." : hospitalInfo.name}
                    </h1>
                </div>
                <p className="text-sm text-gray-500 flex items-center gap-2">
                   <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></span>
                   Govt. System Online • {new Date().toLocaleDateString('en-US', { weekday: 'long', month: 'short', day: 'numeric' })}
                </p>
            </div>
            
            <div className="flex items-center gap-4">
                 <button className="lg:hidden p-2 text-gray-500 hover:bg-gray-100 rounded-lg" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>
                    <Menu size={24} />
                 </button>
                 <div className="hidden md:flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-teal-50 to-cyan-50 border border-teal-100 rounded-full text-teal-700 text-sm font-medium">
        ...
        <div className="flex-1 overflow-auto p-4 lg:p-8">
            ...
            <StatsCard label="Total Patients" value={stats.total} icon={Calendar} color="teal" subtext={timeFilter === "Today" ? "Today" : "Last 7 Days"} />
            <StatsCard label="Pending Action" value={stats.pending} icon={AlertCircle} color="amber" subtext="Requires Assignment" />
            <StatsCard label="Doctors Available" value={stats.activeDoctors} icon={Stethoscope} color="cyan" subtext="Currently On Duty" />
            <StatsCard label="Successful Visits" value={stats.successful} icon={CheckCircle} color="emerald" subtext="Tickets Issued" />
            </div>
            ...
            <div className="p-5 border-b border-gray-100 flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 bg-gray-50/50">
                <div className="flex items-center gap-2">
                    ...
                    <span className="px-2 py-0.5 rounded-md bg-gray-200 text-gray-600 text-xs font-bold">{filteredAppointments.length}</span>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3 w-full xl:w-auto">
                    ...
                    <div className="bg-white p-1 rounded-lg border border-gray-200 flex shadow-sm">
                        <button 
                            ...
                            className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all ${timeFilter === "Today" ? 'bg-teal-50 text-teal-700 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                        >
                            Today
                        </button>
                        <button 
                            onClick={() => setTimeFilter("Week")}
                            className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all flex items-center gap-1 ${timeFilter === "Week" ? 'bg-teal-50 text-teal-700 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                        >
            ...
            <div className="overflow-auto flex-1">
                ...
                <thead className="sticky top-0 bg-white z-10 shadow-sm">
                    ...
                    <th className="p-4 font-semibold bg-gray-50">Assigned Doctor</th>
                    <th className="p-4 font-semibold bg-gray-50">Status</th>
                    <th className="p-4 font-semibold text-right bg-gray-50">Details</th>
                    </tr>
                </thead>
                <tbody className="divide-y divide-gray-50">
                    ...
                    <tr>
                        <td colSpan="7" className="p-20 text-center text-gray-500">
                            <div className="flex flex-col items-center gap-3">
                                ...
                                <span className="font-medium animate-pulse">Syncing with Hospital Database...</span>
                            </div>
                        </td>
                    </tr>
                    ...
                    filteredAppointments.map((apt) => (
                        ...
                        className={`hover:bg-teal-50/40 transition-colors cursor-pointer group ${selectedAppointment?.Id === apt.Id ? 'bg-teal-50/60' : ''}`}
                        >
                        <td className="p-4">
                            <span className="text-sm font-bold text-gray-700 flex items-center gap-2">
                                {apt.Date__c === new Date().toISOString().split('T')[0] ? (
                                    <span className="text-teal-600 font-medium bg-teal-50 px-2 py-0.5 rounded">Today</span>
                                ) : (
                                    apt.Date__c
                                )}
                            </span>
                        ...
                        <td className="p-4">
                            ...
                            <div>
                                <p className="font-semibold text-gray-900">{apt.Patient_Name__c}</p>
                                <p className="text-xs text-gray-500">{apt.Age__c} yrs • {apt.Gender__c}</p>
                            </div>
                            </div>
                        </td>
                        <td className="p-4">
                             <div className="flex items-center gap-1.5">
                                 ...
                                 <span className="text-sm text-gray-700">{apt.Department__c}</span>
                             </div>
                        </td>
                        <td className="p-4">
                        ...
                        <td className="p-4 text-right">
                            <ChevronRight size={18} className="text-gray-300 ml-auto group-hover:text-teal-500 transition-colors" />
                        </td>
                        </tr>
                    ))
                    )}
      ...
      {selectedAppointment && (
        <div className="fixed inset-0 z-50 flex justify-end">
          <div className="absolute inset-0 bg-gray-900/30 backdrop-blur-sm transition-opacity" onClick={() => setSelectedAppointment(null)}></div>
          ...
          <div className="relative w-full max-w-md bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
            ...
            <div className="px-6 py-5 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
              <div>
                ...
                <span className="text-xs font-mono text-gray-400">ID: {selectedAppointment.Id}</span>
              </div>
              <button onClick={() => setSelectedAppointment(null)} className="p-2 hover:bg-gray-200 rounded-full text-gray-500 transition-colors">
                <XCircle size={22} />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
                 {/* Date Warning */}
                 {selectedAppointment.Date__c !== new Date().toISOString().split('T')[0] && (
                    <div className="p-3 bg-amber-50 border border-amber-100 rounded-xl flex gap-3 items-start">
                        <History className="text-amber-500 shrink-0 mt-0.5" size={18} />
                        <div>
                            <p className="text-sm font-bold text-amber-800">Past Appointment</p>
                            <p className="text-xs text-amber-600">This appointment was scheduled for {selectedAppointment.Date__c}. Verify patient details carefully.</p>
                        </div>
                    </div>
                )}
                ...
                <div className="bg-gradient-to-br from-teal-500 to-cyan-600 p-5 rounded-2xl shadow-lg text-white relative overflow-hidden">
                    ...
                    <div className="relative flex items-center gap-4">
                        ...
                        <div>
                        <h4 className="text-xl font-bold">{selectedAppointment.Patient_Name__c}</h4>
                        <p className="text-teal-50 opacity-90 text-sm">{selectedAppointment.Gender__c}, {selectedAppointment.Age__c} years</p>
                ...
                <div className="p-4 bg-teal-50/50 border border-teal-100 rounded-xl flex flex-col gap-1">
                    <label className="text-xs text-teal-600 font-bold uppercase tracking-wider">Aadhaar Number</label>
                    <div className="flex items-center gap-3">
                         ...
                         <span className="ml-auto text-xs px-2 py-1 bg-green-100 text-green-700 font-bold rounded">VERIFIED</span>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 gap-4">
                    <div className="p-3 border border-gray-100 rounded-xl bg-gray-50">
                        <label className="text-xs text-gray-500 font-medium uppercase">Date of Visit</label>
                        <p className="font-semibold text-gray-800 flex items-center gap-2 mt-1">
                            <Calendar size={16} className="text-teal-500" /> {selectedAppointment.Date__c}
                        </p>
                    </div>
                </div>
                ...
                <div>
                     <h4 className="text-sm font-bold text-gray-900 mb-3 flex items-center gap-2">
                         <Stethoscope size={16} className="text-teal-600" /> Clinical Details
                     </h4>
                     <div className="border border-gray-200 rounded-xl overflow-hidden">
                         <div className="p-3 bg-gray-50 border-b border-gray-100 flex justify-between items-center">
                             <span className="text-sm text-gray-600">Department</span>
                             <span className="text-sm font-semibold text-gray-900">{selectedAppointment.Department__c}</span>
                         </div>
                         
                         <div className="p-4 bg-white">
                            {selectedAppointment.Status__c === 'Pending' ? (
                                <div className="space-y-3">
                                    <label className="text-xs font-bold text-gray-500 uppercase">Assign Doctor</label>
                                    <select 
                                    ...
                                    >
                                        ...
                                        {getDoctorsForDepartment(selectedAppointment.Department__c).map(doc => (
                                            <option key={doc.Id} value={doc.Id} disabled={doc.Status__c !== 'Available'}>
                                                {doc.Name} ({doc.Status__c})
                                            </option>
                                        ))}
                                    </select>
                                    {getDoctorsForDepartment(selectedAppointment.Department__c).length === 0 && (
                                        <p className="text-xs text-red-500 flex items-center gap-1 bg-red-50 p-2 rounded">
                                            <AlertCircle size={12}/> No doctors available currently.
                                        </p>
                                    )}
                                </div>
                            ) : (
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p className="text-sm font-bold text-gray-900">{selectedAppointment.Doctor_Name_Snapshot || "Unknown"}</p>
                                        <p className="text-xs text-teal-600 font-medium">Attending Physician</p>
                                    </div>
    ...
    return (
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
            <div className="flex justify-between items-start mb-2">
                ...
                {subtext && <span className="text-xs font-medium text-gray-400 bg-gray-50 px-2 py-1 rounded-lg">{subtext}</span>}
            </div>
